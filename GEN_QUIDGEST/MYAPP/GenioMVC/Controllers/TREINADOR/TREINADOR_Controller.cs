...
namespace GenioMVC.Controllers
{
	public partial class TreinadorController : ControllerBase
	{
		...
		public TreinadorController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL SQB CONTROLLER_NAVIGATION TREINADOR]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAtreinador>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL SQB MANUAL_CONTROLLER TREINADOR]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Treinador row = new Models.Treinador(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("treinador");
			...
			try
			{
				...
				{
					case "TREINADOR__CLUBE__NOME":	// Field (DB)
						{
							var model = new Treinador_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Treinador__clube__nome(qs);
							result = model.TableClubeNome;
						}
						break;
					default:
						break;
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "TREINADOR__CLUBE__NOME":	// Field (DB)
						values = new Treinador_ViewModel(UserContext.Current).GetDependant_TreinadorTableClubeNome(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Treinador" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Treinador([FromBody]Treinador_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "treinador",
				(primaryKey) => Models.Treinador.Find(primaryKey, UserContext.Current, "FTREINADOR"),
				(model) => formData.MapToModel(model as Models.Treinador)
			);
		}
		...
		{
			return base.GetDocumsTickets("TREINADOR", requestModel.FieldName, requestModel.KeyValue);
		}
