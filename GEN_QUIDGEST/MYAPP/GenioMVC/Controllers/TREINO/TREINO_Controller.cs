...
namespace GenioMVC.Controllers
{
	public partial class TreinoController : ControllerBase
	{
		...
		public TreinoController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL SQB CONTROLLER_NAVIGATION TREINO]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAtreino>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL SQB MANUAL_CONTROLLER TREINO]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Treino row = new Models.Treino(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("treino");
			...
			try
			{
				...
				{
					case "TREINO__TREINADOR__NOME":	// Field (DB)
						{
							var model = new Treino_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Treino__treinador__nome(qs);
							result = model.TableTreinadorNome;
						}
						break;
					default:
						break;
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "TREINO__TREINADOR__NOME":	// Field (DB)
						values = new Treino_ViewModel(UserContext.Current).GetDependant_TreinoTableTreinadorNome(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Treino" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Treino([FromBody]Treino_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "treino",
				(primaryKey) => Models.Treino.Find(primaryKey, UserContext.Current, "FTREINO"),
				(model) => formData.MapToModel(model as Models.Treino)
			);
		}
		...
		{
			return base.GetDocumsTickets("TREINO", requestModel.FieldName, requestModel.KeyValue);
		}
