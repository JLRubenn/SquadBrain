<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-treinador-clube-nome-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['TREINADOR__CLUBE__NOME_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'TreinadorClubeNomeSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('TREINADOR__CLUBE__NOME_SeeMore', false),

				interfaceMetadata: {
					id: 'TREINADOR__CLUBE__NOME_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'TREINADOR__CLUBE__NOME_SeeMore',
					name: 'TREINADOR__CLUBE__NOME_SeeMore',
					controller: 'TREINADOR',
					action: 'TREINADOR_ClubeValNome'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-treinador-clube-nome',
				dismissAction: this.close,
				returnElement: 'TREINADOR__CLUBE__NOME_see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.CLUBES32887),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-treinador-clube-nome')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Treinador_Treinador_ClubeValNome',
