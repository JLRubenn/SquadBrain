...
namespace GenioMVC.Controllers
{
	public partial class JogadorController : ControllerBase
	{
		...
		public JogadorController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL SQB CONTROLLER_NAVIGATION JOGADOR]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAjogador>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL SQB MANUAL_CONTROLLER JOGADOR]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Jogador row = new Models.Jogador(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("jogador");
			...
			try
			{
				...
				{
					case "JOGADOR_CLUBENOME____":	// Field (DB)
						{
							var model = new Jogador_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Jogador_clubenome____(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "JOGADOR_CLUBENOME____":	// Field (DB)
						values = new Jogador_ViewModel(UserContext.Current).GetDependant_JogadorTableClubeNome(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Jogador" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Jogador([FromBody]Jogador_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "jogador",
				(primaryKey) => Models.Jogador.Find(primaryKey, UserContext.Current, "FJOGADOR"),
				(model) => formData.MapToModel(model as Models.Jogador)
			);
		}
		...
		{
			return base.GetDocumsTickets("JOGADOR", requestModel.FieldName, requestModel.KeyValue);
		}
