<template>
	...
	<qtable 
		...
		<template #SQB="props">
			<q-badge
				v-for="userRole in props.row.UserRoles.filter(role => role.Module === 'SQB' && Modules[role.Module].active)"
				class="q-table__cell--multiple"
				:key="userRole.Role"
				variant="bold" >
				{{ Resources[userRole.Designation] }}
	...
	export default {
		...
		data() {
			...
			return {
				...
				userId: '',
				Modules: {
					SQB : { active: true, Cod: "SQB", Description: 'MY_APPLICATION56216'},
				},
				Users: {
					rows: [],
					total_rows: 0,
		...
		computed: {
			selectModules: {
                get() {
					...
					return                     this.Modules.SQB.active ;
                },
                set(value) {
                    //If we explicitly declare the variables, vue will bind them
                    this.Modules.SQB.active = value;
                    return value;
                }
            },
		...
		methods: {
			...
			userColumns() {
				...
				{
					label: () => vm.$t(vm.Modules.SQB.Description),
					name: "SQB",
					slot_name: "SQB",
					sort: false,
					visibility: vm.Modules.SQB.active && vm.columnModules
				},
			];
		},
		ImportarUsersAD() {
