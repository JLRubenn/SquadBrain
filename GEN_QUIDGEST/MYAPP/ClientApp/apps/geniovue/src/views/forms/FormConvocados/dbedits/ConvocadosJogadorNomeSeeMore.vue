<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-convocados-jogador-nome-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['CONVOCADOS__JOGADOR__NOME_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'ConvocadosJogadorNomeSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('CONVOCADOS__JOGADOR__NOME_SeeMore', false),

				interfaceMetadata: {
					id: 'CONVOCADOS__JOGADOR__NOME_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'CONVOCADOS__JOGADOR__NOME_SeeMore',
					name: 'CONVOCADOS__JOGADOR__NOME_SeeMore',
					controller: 'CONVOCATORIA',
					action: 'CONVOCADOS_JogadorValNome'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-convocados-jogador-nome',
				dismissAction: this.close,
				returnElement: 'CONVOCADOS__JOGADOR__NOME_see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.JOGADORES08991),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-convocados-jogador-nome')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Convocados_JogadorValNome',
							serverMode: true,
						...
						uuid: 'Convocados_Convocados_JogadorValNome',
