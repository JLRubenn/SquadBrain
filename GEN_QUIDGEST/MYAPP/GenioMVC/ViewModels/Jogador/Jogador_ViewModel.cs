...
{
	public class Jogador_ViewModel : FormViewModel<Models.Jogador>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		/// </summary>
		public decimal? ValNumerocamisola { get; set; }
		/// <summary>
		/// Title: "Nome" | Type: "C"
		/// </summary>
		...
		/// <summary>
		/// Title: "Posição Habitual" | Type: "AC"
		/// </summary>
		...
		public Jogador_ViewModel() : base(null!) { }

		public Jogador_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FJOGADOR", nestedForm) { }

		public Jogador_ViewModel(UserContext userContext, Models.Jogador row, bool nestedForm = false) : base(userContext, "FJOGADOR", row, nestedForm) { }

		public Jogador_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("jogador", id);
			Model = Models.Jogador.Find(id, userContext, "FJOGADOR", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Jogador model = new Models.Jogador(userContext) { Identifier = "FJOGADOR" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FJOGADOR");
			if (navigation != null)
		...
		public override void MapFromModel(Models.Jogador m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Jogador) to ViewModel (Jogador) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error("Map Model (Jogador) to ViewModel (Jogador) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Jogador m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (Jogador) to Model (Jogador) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Jogador) to Model (Jogador) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
		...
		public override void SetViewModelValue(string fullFieldName, object value)
		{
			try
			{
				...
				{
					case "jogador.codclube":
						this.ValCodclube = ViewModelConversion.ToString(_value);
						break;
					case "jogador.foto":
						this.ValFoto = ViewModelConversion.ToImage(_value);
						break;
					case "jogador.numerocamisola":
						this.ValNumerocamisola = ViewModelConversion.ToNumeric(_value);
						break;
					case "jogador.nome":
					...
					case "jogador.datanascimento":
						...
						break;
					case "jogador.pedominante":
						this.ValPedominante = ViewModelConversion.ToString(_value);
						break;
					...
					case "jogador.codjogador":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Jogador) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Jogador)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Jogador.Find(id ?? Navigation.GetStrValue("jogador"), m_userContext, "FJOGADOR"); }
			finally { Model ??= new Models.Jogador(m_userContext) { Identifier = "FJOGADOR" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Jogador.Find(Navigation.GetStrValue("jogador"), m_userContext, "FJOGADOR");
			}
			finally
			{
				if (Model == null)
		...
		protected override void LoadDocumentsProperties(Models.Jogador row)
		{
		}
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Jogador(m_userContext) { Identifier = "FJOGADOR" };
			...
			Load_Jogador_clubenome____(qs, lazyLoad);

// USE /[MANUAL SQB VIEWMODEL_LOADPARTIAL JOGADOR]/
		}

// USE /[MANUAL SQB VIEWMODEL_NEW JOGADOR]/
		...
		public override CrudViewModelValidationResult Validate()
		{
			...
			validator.Required("ValNumerocamisola", Resources.Resources.NUMERO_CAMISOLA34511, ViewModelConversion.ToNumeric(ValNumerocamisola), FieldType.NUMERIC.GetFormatting());
			...
			validator.Required("ValPedominante", Resources.Resources.PE_DOMINANTE49350, ViewModelConversion.ToString(ValPedominante), FieldType.ARRAY_TEXT.GetFormatting());

			validator.Required("ValPosicao", Resources.Resources.POSICAO_HABITUAL63592, ViewModelConversion.ToString(ValPosicao), FieldType.ARRAY_TEXT.GetFormatting());
			validator.StringLength("ValEquipaanterior", Resources.Resources.EQUIPA_ANTERIOR39393, ValEquipaanterior, 50);
...
// USE /[MANUAL SQB VIEWMODEL_SAVE JOGADOR]/
...
// USE /[MANUAL SQB VIEWMODEL_APPLY JOGADOR]/

// USE /[MANUAL SQB VIEWMODEL_DUPLICATE JOGADOR]/

// USE /[MANUAL SQB VIEWMODEL_DESTROY JOGADOR]/
		public override void Destroy(string id)
		{
			Model = Models.Jogador.Find(id, m_userContext, "FJOGADOR");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Jogador_clubenome____(NameValueCollection qs, bool lazyLoad = false)
		{
			bool jogador_clubenome____DoLoad = true;
			CriteriaSet jogador_clubenome____Conds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("clube", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					jogador_clubenome____Conds.Equal(CSGenioAclube.FldCodclube, hValue);
			...
			if (lazyLoad)
			{
				...
				FillDependant_JogadorTableClubeNome(lazyLoad);
				return;
			}

			if (jogador_clubenome____DoLoad)
			{
				...
				jogador_clubenome____Conds.SubSet(search_filters);
...
// USE /[MANUAL SQB OVERRQ JOGADOR_CLUBENOME]/
				...
				*/
				if (Navigation.checkFormMode("clube", FormMode.New) || Navigation.checkFormMode("clube", FormMode.Duplicate))
					jogador_clubenome____Conds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAclube.FldZzstate, 0)
						.Equal(CSGenioAclube.FldCodclube, Navigation.GetStrValue("clube")));
				else
					jogador_clubenome____Conds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAclube.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("clube", "nome");
				ListingMVC<CSGenioAclube> listing = Models.ModelBase.Where<CSGenioAclube>(m_userContext, false, jogador_clubenome____Conds, fields, offset, numberItems, sorts, "LED_JOGADOR_CLUBENOME____", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableClubeNome.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Clube(m_userContext, r, true, _fieldsToSerialize_JOGADOR_CLUBENOME____));
				...
				FillDependant_JogadorTableClubeNome();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_JogadorTableClubeNome(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_JogadorTableClubeNome(bool lazyLoad = false)
		{
			var row = GetDependant_JogadorTableClubeNome(this.ValCodclube);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_JOGADOR_CLUBENOME____ = ["Clube", "Clube.ValCodclube", "Clube.ValZzstate", "Clube.ValNome"];

		protected override object GetViewModelValue(string identifier, object modelValue)
		{
			return identifier switch
			{
				"jogador.codclube" => ViewModelConversion.ToString(modelValue),
		...
		{
			if (ValFoto != null)
				ValFoto.Ticket = Helpers.Helpers.GetFileTicket(m_userContext.User, CSGenio.business.Area.AreaJOGADOR, CSGenioAjogador.FldFoto.Field, null, ValCodjogador);
		}
...
// USE /[MANUAL SQB VIEWMODEL_CUSTOM JOGADOR]/
