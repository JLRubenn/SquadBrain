<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-treino-treinador-nome-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['TREINO__TREINADOR__NOME_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'TreinoTreinadorNomeSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('TREINO__TREINADOR__NOME_SeeMore', false),

				interfaceMetadata: {
					id: 'TREINO__TREINADOR__NOME_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'TREINO__TREINADOR__NOME_SeeMore',
					name: 'TREINO__TREINADOR__NOME_SeeMore',
					controller: 'TREINO',
					action: 'TREINO_TreinadorValNome'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-treino-treinador-nome',
				dismissAction: this.close,
				returnElement: 'TREINO__TREINADOR__NOME_see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.TREINADORES03133),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-treino-treinador-nome')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Treino_Treino_TreinadorValNome',
