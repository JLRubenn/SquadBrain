...
{
	public class Presenca_ValPresenca_ViewModel : MenuListViewModel<Models.Presenca>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<Presenca_ValPresenca_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "Presenca_ValPresenca";
		...
		{
// USE /[MANUAL SQB LIST_LIMITS PRESENCA_PSEUDPRESENCA]/
		...
		public Presenca_ValPresenca_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="Presenca_ValPresenca_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Presenca_ValPresenca_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public Presenca_ValPresenca_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<Presenca_ValPresenca_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Presenca.AddEPH<CSGenioApresenca>(ref u, crs, "IBL_PRESENCAPSEUDPRESENCA");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Presenca.AddEPH<CSGenioApresenca>(ref u, null, "IBL_PRESENCAPSEUDPRESENCA"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioApresenca> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet presencapseudpresencaConds = CriteriaSet.And();
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_PRESENCAPSEUDPRESENCA");
			...
			conditions.SubSets.Add(presencapseudpresencaConds);
			presencapseudpresencaConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL SQB OVERRQ PRESENCA_PSEUDPRESENCA]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioApresenca>(m_userContext, false, ref presencapseudpresencaConds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PRESENCAPSEUDPRESENCA", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL SQB OVERRQLSTEXP PRESENCA_PSEUDPRESENCA]/
			...
			if (tableReload)
			{
// USE /[MANUAL SQB OVERRQLIST PRESENCA_PSEUDPRESENCA]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioApresenca.GetInformation(), QMVC_POS_RECORD, sorts, presencapseudpresencaConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioApresenca> listing = Models.ModelBase.Where<CSGenioApresenca>(m_userContext, distinct, presencapseudpresencaConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PRESENCAPSEUDPRESENCA", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
				...
				Menu.Elements = MapPresenca_ValPresenca(listing);

				Menu.Identifier = "IBL_PRESENCAPSEUDPRESENCA";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_PRESENCAPSEUDPRESENCA";
		...
		private List<Presenca_ValPresenca_RowViewModel> MapPresenca_ValPresenca(ListingMVC<CSGenioApresenca> Qlisting)
		{
			List<Presenca_ValPresenca_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapPresenca_ValPresenca(row));
					i++;
				}
			}
		...
		/// to a Presenca_ValPresenca_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private Presenca_ValPresenca_RowViewModel MapPresenca_ValPresenca(CSGenioApresenca row)
		{
			var model = new Presenca_ValPresenca_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL SQB VIEWMODEL_CUSTOM PRESENCA_VALPRESENCA]/
