...
{
	public class Presenca_ViewModel : FormViewModel<Models.Presenca>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Presenca_ViewModel() : base(null!) { }

		public Presenca_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FPRESENCA", nestedForm) { }

		public Presenca_ViewModel(UserContext userContext, Models.Presenca row, bool nestedForm = false) : base(userContext, "FPRESENCA", row, nestedForm) { }

		public Presenca_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("presenca", id);
			Model = Models.Presenca.Find(id, userContext, "FPRESENCA", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Presenca model = new Models.Presenca(userContext) { Identifier = "FPRESENCA" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FPRESENCA");
			if (navigation != null)
		...
		public override void MapFromModel(Models.Presenca m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Presenca) to ViewModel (Presenca) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			try
			{
				...
				ValEstado = ViewModelConversion.ToString(m.ValEstado);
				ValCodpresenca = ViewModelConversion.ToString(m.ValCodpresenca);
			}
			catch (Exception)
			{
				CSGenio.framework.Log.Error("Map Model (Presenca) to ViewModel (Presenca) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Presenca m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (Presenca) to Model (Presenca) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Presenca) to Model (Presenca) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "presenca.estado":
						...
						break;
					case "presenca.codpresenca":
						this.ValCodpresenca = ViewModelConversion.ToString(_value);
						break;
					default:
						Log.Error($"SetViewModelValue (Presenca) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Presenca)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Presenca.Find(id ?? Navigation.GetStrValue("presenca"), m_userContext, "FPRESENCA"); }
			finally { Model ??= new Models.Presenca(m_userContext) { Identifier = "FPRESENCA" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Presenca.Find(Navigation.GetStrValue("presenca"), m_userContext, "FPRESENCA");
			}
			finally
			{
				if (Model == null)
		...
		protected override void LoadDocumentsProperties(Models.Presenca row)
		{
		}
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Presenca(m_userContext) { Identifier = "FPRESENCA" };
...
// USE /[MANUAL SQB VIEWMODEL_NEW PRESENCA]/
		...
		public override CrudViewModelValidationResult Validate()
		{
			...
			validator.Required("ValCodjogador", Resources.Resources.NOME47814, ViewModelConversion.ToString(ValCodjogador), FieldType.KEY_INT.GetFormatting());

			validator.Required("ValEstado", Resources.Resources.ESTADO07788, ViewModelConversion.ToString(ValEstado), FieldType.ARRAY_TEXT.GetFormatting());
...
// USE /[MANUAL SQB VIEWMODEL_SAVE PRESENCA]/
...
// USE /[MANUAL SQB VIEWMODEL_APPLY PRESENCA]/

// USE /[MANUAL SQB VIEWMODEL_DUPLICATE PRESENCA]/

// USE /[MANUAL SQB VIEWMODEL_DESTROY PRESENCA]/
		public override void Destroy(string id)
		{
			Model = Models.Presenca.Find(id, m_userContext, "FPRESENCA");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
...
// USE /[MANUAL SQB OVERRQ PRESENCA_TREINODATA]/
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Presenca__jogador__nome(NameValueCollection qs, bool lazyLoad = false)
		{
			bool presenca__jogador__nomeDoLoad = true;
			CriteriaSet presenca__jogador__nomeConds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("jogador", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					presenca__jogador__nomeConds.Equal(CSGenioAjogador.FldCodjogador, hValue);
			...
			if (lazyLoad)
			{
				...
				FillDependant_PresencaTableJogadorNome(lazyLoad);
				return;
			}

			if (presenca__jogador__nomeDoLoad)
			{
				List<ColumnSort> sorts = [];
				...
				presenca__jogador__nomeConds.SubSet(search_filters);
...
// USE /[MANUAL SQB OVERRQ PRESENCA_JOGADORNOME]/
				...
				*/
				if (Navigation.checkFormMode("jogador", FormMode.New) || Navigation.checkFormMode("jogador", FormMode.Duplicate))
					presenca__jogador__nomeConds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAjogador.FldZzstate, 0)
						.Equal(CSGenioAjogador.FldCodjogador, Navigation.GetStrValue("jogador")));
				else
					presenca__jogador__nomeConds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAjogador.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("jogador", "nome");
				ListingMVC<CSGenioAjogador> listing = Models.ModelBase.Where<CSGenioAjogador>(m_userContext, false, presenca__jogador__nomeConds, fields, offset, numberItems, sorts, "LED_PRESENCA__JOGADOR__NOME", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableJogadorNome.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Jogador(m_userContext, r, true, _fieldsToSerialize_PRESENCA__JOGADOR__NOME));
				...
				FillDependant_PresencaTableJogadorNome();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_PresencaTableJogadorNome(string PKey)
		{
			FieldRef[] refDependantFields = [CSGenioAjogador.FldCodjogador, CSGenioAjogador.FldNome];
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_PresencaTableJogadorNome(bool lazyLoad = false)
		{
			var row = GetDependant_PresencaTableJogadorNome(this.ValCodjogador);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_PRESENCA__JOGADOR__NOME = ["Jogador", "Jogador.ValCodjogador", "Jogador.ValZzstate", "Jogador.ValNome"];

		protected override object GetViewModelValue(string identifier, object modelValue)
		{
			return identifier switch
			{
				"presenca.codjogador" => ViewModelConversion.ToString(modelValue),
...
// USE /[MANUAL SQB VIEWMODEL_CUSTOM PRESENCA]/
