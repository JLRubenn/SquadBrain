<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-presenca-treino-data-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['PRESENCA__TREINO__DATA_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'PresencaTreinoDataSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('PRESENCA__TREINO__DATA_SeeMore', false),

				interfaceMetadata: {
					id: 'PRESENCA__TREINO__DATA_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'PRESENCA__TREINO__DATA_SeeMore',
					name: 'PRESENCA__TREINO__DATA_SeeMore',
					controller: 'PRESENCA',
					action: 'PRESENCA_TreinoValData'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-presenca-treino-data',
				dismissAction: this.close,
				returnElement: 'PRESENCA__TREINO__DATA_see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.TREINOS59076),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-presenca-treino-data')
		},
